...
## Notes
To publish the page, use this:
```
curl --request POST \
  --url https://4191536-616olivepuffin.adobeioruntime.net/api/v1/web/byom-cc/webhook \
  --header 'Content-Type: application/json' \
  --header 'User-Agent: insomnia/12.2.0' \
  --data '{
	"PROJECT_COORDS":"adobedrago/citibank/main",
	"TOKEN":"<sending in for now, other option available",
	"cfpath":"/content/dam/cf-services/ccc-cf/citi-strata-elite-card",
	"variant":"abc"
}'
```
resulting page url in response with construct:  content fragment name followed by dash and variant
